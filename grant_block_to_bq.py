...
import os
import logging
import grand_block_row_pb2
import glob
import re
from time import sleep

def create_row_data(row_json,dt):
    row = grand_block_row_pb2.grantBlockMsg()
...
if __name__ == '__main__':
    ...
    # ]
    #
    # table = bigquery.Table(table_ref='agolis-allen-first.experiment.grantblock',schema=schema)
    # client.delete_table(table=table,not_found_ok=True)
    # table=client.create_table(table=table,exists_ok=True)
    # exit(0)

    write_client=bigquery_storage_v1.BigQueryWriteClient()
    parent=write_client.table_path('agolis-allen-first','experiment','grantblock')
    ...
    grand_block_row_pb2.grantBlockMsg.DESCRIPTOR.CopyToProto(proto_descriptor)
    ...
    path='/Users/wangez/Desktop/app_access_log/grantblock_*.txt'
    offset=0
    for filename in glob.glob(path):
        dt=re.findall(r"grantblock_(.*?).txt", filename)[0]

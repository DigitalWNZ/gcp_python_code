...
import os
import logging
import ssoaccess_row_pb2
import glob
import re
from time import sleep

def create_row_data(row_json,dt):
    row = ssoaccess_row_pb2.ssoAccessMsg()
    ...
    for item in body:
        element= row.rec.add()
        element.authLevel=item['authLevel']
        element.geoCity=item['geoCity']
        element.geoCountry=item['geoCountry']
        element.geoRegion = item['geoRegion']
        element.ip = item['ip']
        element.userAgent = item['userAgent']
...
if __name__ == '__main__':
    ...
    #                          fields=[
    #                              bigquery.SchemaField("authLevel", "string", mode="nullable"),
    #                              bigquery.SchemaField("geoCity", "string", mode="nullable"),
    #                              bigquery.SchemaField("geoCountry", "string", mode="nullable"),
    #                              bigquery.SchemaField("geoRegion", "string", mode="nullable"),
    ...
    # ]
    #
    # table = bigquery.Table(table_ref='agolis-allen-first.experiment.ssoaccess',schema=schema)
    # client.delete_table(table=table,not_found_ok=True)
    # table=client.create_table(table=table,exists_ok=True)
    # exit(0)

    write_client=bigquery_storage_v1.BigQueryWriteClient()
    parent=write_client.table_path('agolis-allen-first','experiment','ssoaccess')
    ...
    ssoaccess_row_pb2.ssoAccessMsg.DESCRIPTOR.CopyToProto(proto_descriptor)
    ...
    path='/Users/wangez/Desktop/app_access_log/ssoaccess_*.txt'
    offset=0
    for filename in glob.glob(path):
        dt=re.findall(r"ssoaccess_(.*?).txt", filename)[0]
